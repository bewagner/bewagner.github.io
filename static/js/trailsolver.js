      ...
      return {
        ...
        Z3: {
          mk_context: function(c) {
            let ctx2 = Mod._Z3_mk_context(c);
            Mod._set_noop_error_handler(ctx2);
            return ctx2;
          },
          mk_context_rc: function(c) {
            let ctx2 = Mod._Z3_mk_context_rc(c);
            Mod._set_noop_error_handler(ctx2);
            return ctx2;
          },
    ...
    function createApi(Z3) {
      ...
      function createContext(name, options) {
        ...
        function _assertContext(...ctxs) {
          ctxs.forEach((other) => (0, utils_1.assert)("ctx" in other ? ctx2 === other.ctx : ctx2 === other, "Context mismatch"));
        }
        ...
        function throwIfError() {
          if (Z3.get_error_code(contextPtr) !== low_level_1.Z3_error_code.Z3_OK) {
            throw new Error(Z3.get_error_msg(ctx2.ptr, Z3.get_error_code(ctx2.ptr)));
          }
        }
        function check(val) {
          throwIfError();
        ...
        async function solve2(...assertions) {
          const solver = new ctx2.Solver();
        ...
        const Datatype = Object.assign((name2) => {
          return new DatatypeImpl(ctx2, name2);
        }, {
        ...
        function And(...args) {
          if (args.length == 1 && args[0] instanceof ctx2.AstVector) {
            args = [...args[0].values()];
        ...
        class SolverImpl {
          ...
          toSmtlib2(status = "unknown") {
            ...
            if (numAssertions > 0) {
              ...
              assumptions = [];
              formula = ctx2.Bool.val(true).ast;
            }
        ...
        cleanup.register(ctx2, () => Z3.del_context(contextPtr));
        return ctx2;
      }
      return {
        enableTrace,
...
async function ensureCtx() {
  ...
  const { Context } = await (0, import_z3_solver.init)();
  ctx = Context("main");
  return ctx;
}
